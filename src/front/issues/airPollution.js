...
function AirPollution() {
  ...
  async function likeChange(e) {
    ...
    if (!likedComments.includes(idi)) {
      ...
      setLikedComments([...likedComments, idi]);
    }
  }
  ...
  return (
    <div className={styles.body}>
            ...
            comments.map((comment, i) => (
              <div key={comment.id} className={styles.usercomment} style={{ minHeight: '200px' }}>
                ...
                <div className={styles.like}>
                  <button className={styles.combutton} idi={comment.id} value={comment.likes} onClick={likeChange} disabled={likedComments.includes(comment.id)}>
      ...
      <footer className={styles.footer1}><h1>Footer</h1></footer>
    </div>
  );
}
